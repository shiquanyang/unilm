...
class MultimodalKB(nn.Module):
    def __init__(self, hidden_size, lang, max_response_len, path, task, lr, n_layers, dropout, input_channels, output_channels, conv_kernel_size, pool_kernel_size):
        super(MultimodalKB, self).__init__()
        self.name = "MultimodalKB"
        self.task = task
    ...
    def print_loss(self):
        ...
        self.print_every += 1
        return 'L:{:.2f},V:{:.2f},C:{:.2f}'.format(print_loss, print_loss_v, print_loss_c)
    ...
    def reset(self):
        self.print_every, self.loss, self.loss_v, self.loss_c = 1, 0, 0, 0
    ...
    def encode_and_decode(self, data, max_target_length, use_teacher_forcing, get_decoded_words):
        ...
        calibration_vocab = self.calibration(dh_hidden)
    ...
    def evaluate(self, dev, matric_best, early_stop=None):
        ...
        for j, data_dev in pbar:
            # Encode and Decode
            _, decoded_fine, decoded_coarse, calibration_vocab = self.encode_and_decode(data_dev, self.max_resp_len, False, True)
